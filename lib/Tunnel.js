...
export default class Tunnel extends EventEmitter {
  ...
  _getInfo(body) {
    ...
    const { host, port: local_port, local_alias } = this.opts;
      ...
      axios
        ...
        .then((res) => {
          ...
          if (res.status !== 200) {
            const err = new Error(
              (body && body.message) ||
                "quicknexus returned an error, please try again"
            );
            return cb(err);
          }
  ...
  _establish(info) {
    // increase max event listeners so that quicknexus consumers don't get
